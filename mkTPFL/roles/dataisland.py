...
class DataIsland:
    ...
    Attributes:
        ...
        sv_threshold: minimum number of SVs needed to recover secrets。
    ...
    def iterkeyGen(self, update_flag=False):
        ...
        # sample random_vec
        random_vec, dec_part = ZZ(N), ZZ(N)
        ring.sampleZO(random_vec)

        # compute dec_part_iteration = rv*rax*sk
        rax, sx = self.enc_public_key.rax, self.enc_secret_key.sx
        ring.multNTT(dec_part, random_vec, rax, np, qQ)
        ring.multAndEqual(dec_part, sx, np, qQ)

        self.enc_rv_iteration = random_vec
        # self.dec_part_iteration = Plaintext(dec_part, logp, logq, self.ckks_params.n)
        # return self.dec_part_iteration
        dec_part_iteration = Plaintext(dec_part, logp, logq, self.ckks_params.n)
        return dec_part_iteration
    #### ------------------------------------------------------------
    #### ------------------------------------------------------------
    ...
    #### -------------------- ENCRYPTION METHODS --------------------
    #### ------------------------------------------------------------ 
    def encrypt(self, message, public_key=None, imag_flag=False, rv_flag=False, random_vec=None, thread_num=8, list_flag=False, n=None, logp=None, logq=None):
    ...
    def partEncrypt(self, message, public_key=None, random_vec=None, thread_num=8):
    ...
    def partEncIterkey(self, iterkey=None, attend_di_index_list=None, aggregation_public_key=None, thread_num=1):
        ''' encrypte iterkey '''
        iterkey = iterkey if iterkey else self.dec_part_iteration
        ...
        partciph = self.partEncrypt(iterkey, thread_num=thread_num) # default 4 threads
        return partciph
    #### ------------------------------------------------------------
    #### ------------------------------------------------------------
    ...
    #### -------------------- DECRYPTION METHODS --------------------
    #### ------------------------------------------------------------ 
    def decrypt(self, ciphertext, sk=None, plain_flag=False, msg_flag=False, list_flag=True):
        ''' '''
        sk = sk if sk else self.enc_secret_key
        decryptor = CKKSDecryptor(self.ckks_params, sk)
    ...
    def verCiphSumMKSign(self, ciph_sum, sign_csum, pks, lhsmsg_ciphs, weights=None):
    ...
    def verGlobalModelMKSign(self, model, gm_soln, sign_gm, pks, hash_lms, weights=None):
        ''' verify SV's signature of hash(gm) using dis' lhs_pk
            ( gm = sum([ lm[i] for i in di_inxs ]) 
        '''
        msg_len, msgs_num = len(gm_soln), len(pks)
