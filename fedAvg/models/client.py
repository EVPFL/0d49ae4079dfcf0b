...
class Client(object):
    ...
    def get_real_eval_result(self, local_verify_data, local_solution, sample_num=1):
        """ Compute mse loss of local model on local verify_data
        """
        verify_dataloader = DataLoader(local_verify_data, batch_size=1, shuffle=False)
        mse_loss = self.worker.local_model_eval(verify_dataloader, local_solution, sample_num)
        return mse_loss
