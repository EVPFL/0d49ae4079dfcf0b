from fedAvg.utils.flops_counter import get_model_complexity_info
from fedAvg.utils.torch_utils import get_flat_grad, get_state_dict, get_flat_params_from, set_flat_params_to
...
class Worker(object):
    ...
    def local_train(self, train_dataloader, **kwargs):
        ...
        # model_weight,model_bias = models['layer.weight'],models['layer.bias']
        # print(model_weight[0].shape)
        # print(model_weight[0][1])
        # print(model_bias.shape)
        # model_stat = self.model.state_dict()
        # model_weight = model_stat['layer.weight']
        # model_bias = model_stat['layer.bias']
        # print('model_weight (local_train): ', model_weight)
        # print('model_bias (local_train): ', model_bias)
    ...
    def local_model_eval(self, verify_dataloader, local_solution, sample_num):
        org_solution = self.get_flat_model_params()
        self.set_flat_model_params(local_solution)
        ...
        # model_weight = model_stat['layer.weight']
        # model_bias = model_stat['layer.bias']
        # print('model_weight (local_eval): ', model_weight)
        # print('model_bias (local_eval): ', model_bias)
        with torch.no_grad():
            for x, y in verify_dataloader:
                count += 1
                if self.gpu:
                    x, y = x.cuda(), y.cuda()
                pred = self.model(x)
                Y = torch.zeros(pred.size())
                for i in range(len(y)):
                    label = y[i]
                    Y[i][label] = 1
                mse_loss += mseloss(pred, Y)
                # print('local_eval_x:', x)
                # print('local_eval_y:', y)
                if count >= sample_num:
                    break
        self.set_flat_model_params(org_solution)
        return mse_loss
