...
__all__ = ['mkdir', 'read_data', 'read_verify_data', 'Metrics', "MiniDataset"]
...
def read_data(train_data_dir, test_data_dir, key=None, verify_path=None, dataset_name=None, model_name=None):
    ...
    for cid, v in train_data.items():
        train_data[cid] = MiniDataset(v['x'], v['y'], dataset_name=dataset_name, model_name=model_name)
    ...
    for cid, v in test_data.items():
        test_data[cid] = MiniDataset(v['x'], v['y'], dataset_name=dataset_name, model_name=model_name)
...
def read_verify_data(verify_data_dir, key=None, dataset_name=None, model_name=None):
    """Parses data in given verify data directories
    Return:
        verify_data: verify data (ndarray)
    """
    verify_data = {}
    print('>>> Read data from:')
    verify_files = os.listdir(verify_data_dir)
    verify_files = [f for f in verify_files if f.endswith('.pkl')]
    if key is not None:
        verify_files = list(filter(lambda x: str(key) in x, verify_files))

    for f in verify_files:
        file_path = os.path.join(verify_data_dir, f)
        print('    ', file_path)

        with open(file_path, 'rb') as inf:
            cdata = pickle.load(inf)
        verify_data.update(cdata['user_data'])

    # assert len(verify_data) == 1
    cid, v = list(verify_data.items())[0]
    verify_data[cid] = MiniDataset(v['x'], v['y'], dataset_name=dataset_name, model_name=model_name)

    return verify_data[cid]




class MiniDataset(Dataset):
    def __init__(self, data, labels, dataset_name, model_name):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == 'mnist' and self.model_name == 'lenet':
